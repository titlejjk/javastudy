 ...
 *  
 *  1. 객체의 설계도 역할
 *     - 객체는 heap 영역(사물함) 에 들어가 있는 물건이라고 생각하면 된다.
 *     - class 를 이용해서 객체를 원하는 시점에 원하는 갯수 만큼 만들어 낼수 있다.
 *     - 만들어진 객체가 어떤 저장소와 어떤 기능을 가지게 할지 class로 설계 할수 있다.
 *     - class명 자체가 data type의 역할이 가능하다.
 *  2. data type 역할
 *  
...
public class Car {
	 ...
	 */
	public void drive() {
		System.out.println(this.price + "원 짜리"+ this.name + "가(이) 달려요!");
	}
	
	
	public void showInfo() {
		System.out.println("차의 이름은 "+ this.name +"이고 가격은 " + this.price + "입니다.");
	}
